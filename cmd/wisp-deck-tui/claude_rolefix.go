...
func init() {
	rootCmd.AddCommand(newClaudeRolefixCommand(runClaudeRolefixChild))
}

// newClaudeRolefixCommand wraps one Claude launch in a loopback proxy that
// repairs the message roles a strict Anthropic endpoint rejects, and points the
// session's settings overlay at it.
//
