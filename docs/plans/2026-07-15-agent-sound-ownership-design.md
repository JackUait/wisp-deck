...
Both the Codex app-server and TUI receive `notify=[]` so a user-level external
notifier command cannot execute audio inside either process. The TUI keeps the exact
...
- the Codex app-server and every fresh/resume, embedded/remote TUI argv disable
  the external notifier with `notify=[]`;
