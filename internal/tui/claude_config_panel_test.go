...
func TestUpdate_clickPlanSettingsRow_opensModelMap(t *testing.T) {
	...
	m.width, m.height = 100, 60
	_ = m.View()
	// Plan (Subscription) sits under the Account section header. Find its body
	// row from the same layout the renderer/mouse mapping use.
	bodyRows := m.settingsBodyRowIndices()
	rowOffset := -1
	for i, idx := range bodyRows {
		if idx == rowSubscription {
			rowOffset = i
			break
		}
	}
	if rowOffset < 0 {
		t.Fatal("Subscription not found in settings body rows")
	}
