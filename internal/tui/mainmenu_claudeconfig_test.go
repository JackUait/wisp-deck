...
func TestSettings_nav_count_includes_config_for_all_agents(t *testing.T) {
	m, _ := newClaudeMenu(t)
	if got := m.settingsItemCount(); got != 11 {
		t.Fatalf("claude should have 11 settings items (incl. Usage bars + AI tools + Plan + Login + Keep awake), got %d", got)
	}
	m.CycleAITool("next")
	if got := m.settingsItemCount(); got != 11 {
		t.Fatalf("non-claude should also have 11 settings items (shared Plan + Login), got %d", got)
	}
}
