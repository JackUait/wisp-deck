#!/bin/bash
# AI tool selection TUI wrapper using wisp-deck-tui
...
select_ai_tool_interactive() {
  ...
  local result
  if ! result=$(wisp-deck-tui multi-select-ai-tool 2>/dev/null); then
    return 1
  fi
  ...
  # Pick first tool by priority: claude > opencode > codex
  local priority_order=("claude" "opencode" "codex")
  _selected_ai_tool=""
