...
func auditedProductionProcessCalls() map[string]int {
	return map[string]int{
		...
		`internal/allin/keychain_darwin.go:keychainBlob:exec.Command("security", "find-generic-password", "-s", KeychainService(configDir), "-w")`:                        1,
		`internal/allin/keychain_darwin.go:keychainAccount:exec.Command("security", "find-generic-password", "-s", KeychainService(configDir))`:                           1,
		`internal/allin/keychain_darwin.go:keychainCLI.write:exec.Command("security", "add-generic-password", "-U", "-a", account, "-s", service, "-w", string(updated))`: 1,
